...
DEFAULT_COUNTRIES = ("gb", "us", "ca", "au", "nz", "za", "sg")
...
def fetch(keywords: list[str], skills: list[str],
          ...
          results_per_page: int = 40, pages: int = 1,
          countries=DEFAULT_COUNTRIES) -> list[Job]:
    ...
    for country in countries:
        ...
        for page in range(1, pages + 1):
            ...
            try:
                ...
                print(f"[adzuna] {country} page {page} failed: {exc}")
                if page == 1:
                    break
                continue
            except ValueError:
            ...
            for row in data.get("results") or []:
                ...
                if not remote:
                    continue
                if "hybrid" in f"{title} {desc_head}".lower():
                    continue
    ...
    print(f"[adzuna] {len(jobs)} remote jobs for query={query!r} across {len(countries)} countries")
    return jobs